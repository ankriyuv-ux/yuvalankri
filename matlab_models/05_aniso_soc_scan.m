function aniso_soc_scan()
% =========================================================================
% STAGE 2: SOC + EXCHANGE + UNIAXIAL ANISOTROPY | FULL XYZ LLG
%
% Goal:
%   Test whether Ku stabilizes the SOC-selected Mz response and reduces
%   temporal cancellation, rather than creating the response by itself.
%
% Fixed:
...
% with e_easy = +Z.  Ku=0 is the built-in no-anisotropy reference.
%
% Key diagnostic:
...
% Ku values are stored in eV.  These correspond to 0...2 meV.
p.KuList_eV = [0 0.00025 0.00050 0.00075 0.00100 0.00125 0.00150 0.00200];
p.easyAxis  = [0 0 1];
...
assert(any(abs(p.KuList_eV)<1e-15),'Ku=0 reference is required.');
assert(all(ismember([5 -5],p.socList_meV)),'SOC list must contain +5 and -5 meV.');

fprintf('\n============================================================\n');
fprintf('STAGE 2 | SOC + EXCHANGE + ANISOTROPY | FULL XYZ LLG\n');
fprintf('============================================================\n');
fprintf('J0                 = %g meV\n',p.J0_meV);
...
outputFolder = fullfile(downloadsDir, ...
    ['SOC_ANISOTROPY_KU_SCAN_J150_FULLXYZ_10PS_' runTag]);
...
%% 8. Ku=0 REFERENCE CHECK ================================================
ik0 = find(abs(p.KuList_eV)<1e-15,1);
fprintf('\nKu=0 references (should match the previous J0=150 Full-XYZ SOC runs):\n');
for isoc = 1:nSOC
    E0 = runs{isoc,ik0};
...
summaryTable = table( ...
    ...
    max_Beff_perp_mT,max_Baniso_mT,max_tilt_deg, ...
    'VariableNames',{ ...
    'SOC_meV','Ku_meV','Mz_end','max_abs_Mz','torque_cancellation_ratio', ...
    'max_Beff_perp_mT','max_Baniso_mT','max_tilt_deg'});

disp(summaryTable);
writetable(summaryTable,fullfile(outputFolder,'SOC_KU_STAGE2_SUMMARY.csv'));
save(fullfile(outputFolder,'SOC_KU_STAGE2_COMPLETE.mat'), ...
    'runs','summaryTable','p','model','initialDiagnostic','-v7.3');

%% 10. FIGURES ============================================================
% A. Mz(t) Ku scan, one figure for each SOC sign.
for isoc = 1:nSOC
    ...
    title(sprintf('SOC = %+g meV | J_0 = 150 meV | Full XYZ | K_u scan',p.socList_meV(isoc)));
    legend('Location','best'); xlim([0 10]);
    save_figure(fig,outputFolder,sprintf('FIG_MZ_SOC_%+gmeV_KU_SCAN.png',p.socList_meV(isoc)));
end

% B. Final Mz versus Ku: checks whether SOC sign control is preserved.
...
title('Does anisotropy stabilize the SOC-selected M_z sign?');
legend('Location','best');
save_figure(fig2,outputFolder,'FIG_MZ_END_VS_KU_SOC_SIGN.png');

% C. True temporal cancellation of the z rotation rate.
fig3 = figure('Color','w','Position',[220 100 1050 700]); hold on;
for isoc = 1:nSOC
    idxRow = summaryTable.SOC_meV == p.socList_meV(isoc);
    plot(summaryTable.Ku_meV(idxRow), ...
        summaryTable.torque_cancellation_ratio(idxRow), ...
        '-o','LineWidth',1.5,'DisplayName',sprintf('SOC = %+g meV',p.socList_meV(isoc)));
end
grid on;
xlabel('K_u [meV]');
ylabel('|int dM_z/dt dt| / int |dM_z/dt| dt');
title('Temporal cancellation of the M_z torque');
legend('Location','best'); ylim([0 1]);
save_figure(fig3,outputFolder,'FIG_TORQUE_CANCELLATION_VS_KU.png');

% D. Max |Mz| versus Ku.
fig4 = figure('Color','w','Position',[220 100 1050 700]); hold on;
for isoc = 1:nSOC
    idxRow = summaryTable.SOC_meV == p.socList_meV(isoc);
    plot(summaryTable.Ku_meV(idxRow),summaryTable.max_abs_Mz(idxRow), ...
        '-o','LineWidth',1.5,'DisplayName',sprintf('SOC = %+g meV',p.socList_meV(isoc)));
end
grid on;
xlabel('K_u [meV]'); ylabel('max |M_z|');
title('Maximum out-of-plane response versus anisotropy');
legend('Location','best');
save_figure(fig4,outputFolder,'FIG_MAX_MZ_VS_KU.png');
